...
class Fish(Agent):
    ...
    def __init__(
        ...
        self.velocity = tuple([random.uniform(-self.max_speed/2, self.max_speed/2)
                              for _ in range(len(pos))])  # initial velocity
    ...
    def align(self) -> List[float]:
        ...
        for i in range(len(self.pos)):
            neighbors_velocity = [n[0].velocity[i]
                                  for n in neighbors if n[1] != 0]
            if len(neighbors_velocity) == 0:
                avg_vel[i] = 0
            else:
                avg_vel[i] = sum(neighbors_velocity) / len(neighbors_velocity)
            align_update[i] = (avg_vel[i] - self.velocity[i]
                               ) * alignment_strength
    ...
    def separation(self) -> List[float]:
        ...
        # new code
        close_neighbors = [n[0]
                           for n in neighbors if n[1] != 0 and n[1] < self.min_dist]

        for cn in close_neighbors:
            for i, pos in enumerate(self.pos):
                separation_update[i] = (
                    pos - cn.pos[i]) * self.separation_strength
    ...
    def towards_food(self) -> List[float]:
        ...
        visible_foods = self.model.get_neighboring(
            self, self.perception, False, self.model.foods)
        ...
        for food in visible_foods:
            ...
            for i in range(len(self.pos)):
                ...
                towards_food_update[i] += direction * \
                    nutritional_value * hunger * attraction
    ...
    def avoid_shark(self) -> List[float]:
        ...
        visible_sharks = self.model.get_neighboring(
            self, self.perception, False, self.model.sharks)
        ...
        for shark in visible_sharks:
            ...
            for i in range(len(self.pos)):
                avoid_shark_update[i] += (self.pos[i] -
                                          shark[0].pos[i]) / shark[1]
    ...
    def limit_velocity(self, velocity) -> List[float]:
        ...
        fixed_velocity = [
            comp * self.max_speed for comp in normalize(tuple(velocity))]
    ...
    def eat(self) -> None:
        ...
        if self.energy < self.max_energy:
            available_foods = self.model.get_neighboring_food(
                self, self.eat_radius)

            for food in available_foods:
                eat_fraction = min(food.available_fraction,
    ...
    def recombine_genes(self, second_parent) -> List[float]:
        ...
        child_genes = [gene * recombine_list[i] +
                       (1 - recombine_list[i]) * second_parent.genes[i] for i, gene in enumerate(self.genes)]
    ...
    def step(self) -> None:
        ...
        # ages
        self.age += 1
        self.neighbors = self.model.get_neighbors_w_distance(
